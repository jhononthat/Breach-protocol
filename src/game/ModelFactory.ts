...
export class ModelFactory {
  ...
  private static getMaterialsForSkin(skin?: WeaponSkin) {
    ...
    const { colors } = skin;
    const isBlackIce = skin.id === 'black_ice';

    const receiverMat = new THREE.MeshStandardMaterial({
      color: isBlackIce ? 0x0c2d48 : colors.primary,
      roughness: isBlackIce ? 0.03 : colors.roughness,
      metalness: isBlackIce ? 0.92 : colors.metalness,
      emissive: isBlackIce ? 0x004466 : (colors.emissive || 0x000000),
      emissiveIntensity: isBlackIce ? 0.5 : (colors.emissive ? 0.85 : 0)
    });

    const fdeMat = new THREE.MeshStandardMaterial({
      color: isBlackIce ? 0xe0f7fc : colors.secondary,
      roughness: isBlackIce ? 0.05 : Math.min(0.9, colors.roughness + 0.12),
      metalness: isBlackIce ? 0.88 : Math.max(0.1, colors.metalness * 0.72)
    });

    const steelMat = new THREE.MeshStandardMaterial({
      color: isBlackIce ? 0x7ae5ff : colors.accent,
      roughness: 0.08,
      metalness: 0.98,
      emissive: isBlackIce ? 0x00d4ff : 0x000000,
      emissiveIntensity: isBlackIce ? 0.6 : 0
    });
