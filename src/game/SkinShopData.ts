...
export class SkinManager {
  ...
  public static rollBlackIcePack(): { skin: WeaponSkin; isDuplicate: boolean; duplicateRenown: number } {
    ...
    if (roll < 10.0) {
      // 10% chance for the coveted Black Ice camo in Black Ice Tactical Pack!
      chosenSkin = WEAPON_SKINS.find(s => s.id === 'black_ice') || WEAPON_SKINS[1];
    } else {
      // High-tier pool (Legendary or Epic)
      const highTier = WEAPON_SKINS.filter(s => s.rarity === 'legendary' || s.rarity === 'epic' || s.id === 'black_ice');
  ...
  public static rollDeltaPack(): { skin: WeaponSkin; isDuplicate: boolean; duplicateRenown: number } {
    ...
    const chosenSkin = candidates[Math.floor(Math.random() * candidates.length)] || WEAPON_SKINS[1];
